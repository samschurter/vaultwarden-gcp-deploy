...
#   None
# Behavior:
#   Creates a compressed tarball of vaultwarden data (attachments, sends, config, rsa_key*)
...
make_backup() {
  ...
  mkdir -p "$BACKUP_DIR"
  BACKUP_FILE=$BACKUP_DIR/"vw_backup_$(date "+%F-%H%M%S").tar.gz"
...
#   None
# Behavior:
#   Attempts to stop the `vaultwarden` Docker container using `docker stop`.
# Returns:
#   0 on success; non-zero if container could not be stopped.
stop_vaultwarden() {
  log "$(printf "Stopping vaultwarden container...")"
  if ! docker stop vaultwarden > /dev/null; then
    log "$(printf "Could not stop vaultwarden container. Restoration may fail if database is in use.")" "WARNING"
    return 1
  fi
  return 0
}
...
#   None
# Behavior:
#   Attempts to start the `vaultwarden` Docker container using `docker start`.
# Returns:
#   0 on success; non-zero if the container failed to start.
start_vaultwarden() {
  log "$(printf "Starting vaultwarden container...")"
  if ! docker start vaultwarden > /dev/null; then
    log "$(printf "Could not start vaultwarden container. You may need to start it manually")" "WARNING"
    return 1
  fi
  return 0
}
...
restore_backup() {
  ...
  # Stop the vaultwarden container before restoration
  ...
  if command -v docker >/dev/null 2>&1; then
    ...
    log "Docker command not found. Cannot stop/start vaultwarden container." "WARNING"
  fi
  ...
  # Restart vaultwarden container if we stopped it
  if [ "$VAULTWARDEN_STOPPED" -eq 1 ] && command -v docker >/dev/null 2>&1; then
    start_vaultwarden
  fi
